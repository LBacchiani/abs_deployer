            ...
            for h in zep_sol["locations"][i][j].keys():
                for k in range(0,zep_sol["locations"][i][j][h]):
                    if h in initial_obj_into_name:
                        ...
                        obj_to_abs_name[(i,int(j),h,k)] = (settings.OBJ_PREFIX + h + "_" + str(k) + "_" + i + "_" + j).replace(".", "_")
...
def get_providers(bindings, deployment):
    dcname,dcnum,objname,objnum = deployment
    prov = {}
    for i in bindings:
        ...
        for j in i:
            ...
            if not objname.startswith(settings.SEPARATOR):
                ...
                for k in interfaces:
                    ...
                    if k == interfaces[0]:
                        undep += "\t\t" + k + " " + obj_to_abs_name[j] + " = nth(ls_" + k + "," + str(counter[k]) + ");\n"
...
def print_cloud_provider_modification(dc_json, out):
    ...
    for i in dc_json.keys():
        if "initial_DC" not in i:
            ...
            dcDescription += '\t\t\tmap['
            dcDescription += 'Pair(CostPerInterval,' + str(dc_json[i]["cost"]) + ")\n"
            dcDescription += '\t\t\t,Pair(PaymentInterval,' + str(dc_json[i]["payment_interval"]) + ")\n"
            for j in dc_json[i]["resources"].keys():
                ######### find max startup duration ############
                if j == "Startupduration":
                    time = int(str(dc_json[i]["resources"][j]))
                    if time > max_time:
                        max_time = time
                ...
                if j != "fictional_res" and j != "Startupduration":
                    dcDescription += '\t\t\t,Pair(' + j + ',' + str(dc_json[i]["resources"][j]) + ")\n"
            dcDescription += '\t\t\t]));\n'
    if max_time > 0:
        global waiting_time
